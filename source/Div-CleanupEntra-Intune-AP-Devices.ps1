<#
.SYNOPSIS
    Two-stage stale-device cleanup for Entra ID -- delegated (interactive) auth.
    Runs as the signed-in admin via Connect-MgGraph -Scopes.

.DESCRIPTION
    See README.md next to this script. The signed-in user needs a role that can
    disable + delete devices (Cloud Device Administrator is the least-priv fit).
    Required delegated Graph scopes are listed in $GraphScopes below; the script
    logs a WARN at startup if any requested scope was not granted.

.PARAMETER Apply
    Forces $DryRun = $false for this run.

.PARAMETER UseDeviceCode
    Use device-code flow instead of a browser pop-up (headless / SSH / PSRemoting).

.PARAMETER LogPath
    Output directory for transcript + CSVs. Defaults to <scriptdir>\logs.
...
.EXAMPLE
    .\Invoke-EntraIdStaleDeviceCleanup-Delegated.ps1
.EXAMPLE
    .\Invoke-EntraIdStaleDeviceCleanup-Delegated.ps1 -Apply -UseDeviceCode
.EXAMPLE
    .\Invoke-EntraIdStaleDeviceCleanup-Delegated.ps1 -DeviceListCsv .\targets.csv -Apply
#>
...
param(
    ...
    [Parameter(Mandatory = $false)] [string] $LogPath       = (Join-Path -Path $PSScriptRoot -ChildPath 'logs'),
...
# --- Auth (delegated / interactive) -----------------------------------------
# Blank uses the built-in "Microsoft Graph PowerShell" app + the user's home tenant.
# Set $ClientId to a custom app reg with admin-consented delegated perms to skip user consent.
$TenantId = ''
$ClientId = ''

# Delegated scopes requested at sign-in. The last two are only needed when
# $DeleteIntuneObjects / $DeleteAutopilotObjects are enabled.
$GraphScopes = @(
...
# --- Run mode ---------------------------------------------------------------
# Default $true for interactive use; -Apply on the cmdline forces $DryRun=$false.
$DryRun         = $false
...
$BackupOnlyBeforeHardDelete = $true
...
$ProceedOnAutopilotLookupFailure = $false
...
$OnDuplicateMatch         = 'Skip'   # 'Skip' or 'ProcessAll'
...
# $TenantId / $ClientId are optional for delegated auth; $GraphScopes is not.
if (-not $GraphScopes -or $GraphScopes.Count -eq 0) {
    throw '$GraphScopes must contain at least one delegated permission scope.'
}
...
try {
    # Build the Connect-MgGraph parameter set dynamically. -Scopes is always
    # passed; -TenantId / -ClientId are only added if the operator set them
    # at the top of the script. -UseDeviceCode swaps the browser pop-up for
    # a code the user can enter on another device.
    $connectArgs = @{
        Scopes      = $GraphScopes
        NoWelcome   = $true
        ErrorAction = 'Stop'
    ...
    $tenantLabel = if ($TenantId) { $TenantId } else { '<home tenant>' }
    $clientLabel = if ($ClientId) { $ClientId } else { '<built-in Graph PowerShell app>' }
    Write-Log ("Connecting to Microsoft Graph as the signed-in user (tenant {0}, client {1})..." -f $tenantLabel, $clientLabel)
    Write-Log ("Requesting delegated scopes: {0}" -f ($GraphScopes -join ', '))
    ...
    # Sanity check: warn if any requested scope is missing from the granted
    # set. Delegated tokens silently drop scopes the user/tenant didn't
    # consent to; the script will then fail later with a 403 on the affected
    # call (e.g. BitLocker fetch) and you won't know why.
    $granted = @($ctx.Scopes)
    $missing = $GraphScopes | Where-Object { $_ -notin $granted }
...
Write-Log ("Safety rails: DisableOnly={0}  RequireDisabledBeforeDelete={1}  PreviewSecretsBackup={2}  BackupBLandLAPs={3}  BackupOnlyBeforeHardDelete={4}  DeleteIntuneObjects={5}  DeleteAutopilotObjects={6}  ProceedOnAutopilotLookupFailure={7}  AutopilotLookupPreferEnumeration={8}  ConfirmActions={9}  DryRun={10}" -f `
    $DisableOnly, $RequireDisabledBeforeDelete, $PreviewSecretsBackup, $BackupBLandLAPs, $BackupOnlyBeforeHardDelete, $DeleteIntuneObjects, $DeleteAutopilotObjects, $ProceedOnAutopilotLookupFailure, $AutopilotLookupPreferEnumeration, $ConfirmActions, $DryRun)
if ($DeviceListCsv) {
...
function Remove-AutopilotDevice {
    ...
    if ($ap.Count -eq 0) {
        # Stale ZTDId in Entra but no AP record -- already clean, let Entra delete proceed.
...
foreach ($device in $staleDevices) {
    ...
    if ($isAutopilot) {
        ...
        if ($DeleteAutopilotObjects -and $targetStage -eq 'HardDelete') {
            ...
            $apDeleteOk = Remove-AutopilotDevice -EntraDeviceId $device.DeviceId -DisplayName $device.DisplayName -DryRun ([bool]$DryRun)
            if ($apDeleteOk) {
