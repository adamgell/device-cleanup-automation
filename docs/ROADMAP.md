...
| 5a | Pretty HTML alert emails (ACS + Logic App, `pretty_email_enabled`) | 3 | **Done** | Live 2026-08-11: direct payload + real alert notifications (incl. Resolved) delivered via action group → Logic App → ACS to inbox. Gotcha: Microsoft.Communication provider must be registered once per subscription. |
